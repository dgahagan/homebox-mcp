...
- [x] 3.5 **Publish (S)** — done 2026-07-03: public repo github.com/dgahagan/homebox-mcp (CI green); PyPI `homebox-mcp` 0.9.0 then 0.9.1 via trusted publishing (uvx-verified from the live index); official MCP registry listing `io.github.dgahagan/homebox-mcp` v0.9.1 (server.json + mcp-name README marker; verified via registry search API). Decisions: version 0.9.x for public shakeout (1.0.0 later); REVIEW/PLAN moved to docs/; both companion scripts kept.
